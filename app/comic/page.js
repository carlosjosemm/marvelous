...
const ComicPage = () => {
  ...
  const getComicData = async (comicId) => {
    ...
    const comicRes = await axios.get(comicBaseURL + comicId, {
      params: {
  ...
  useEffect(() => {
    const hasParam = searcParams?.getAll("comicId").reduce((acc, param) => {
  ...
  return (
    <ComicContainer>
      <ComicThumbnail
        src={`${comicData.thumbnail.path.replace("http:", "https:")}.${
