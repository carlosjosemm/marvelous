...
const Header = () => {
  ...
  const handleSubmit = (ev) => {
    ...
    if (!isAdvancedSearch) {
      ...
      if (
        value.includes("https://www.marvel.com/comics/issue") ||
        value.includes("http://www.marvel.com/comics/issue")
      ) {
      ...
      const url = `/search?${getQuery(heroes, "hero")}${getQuery(
        comics,
        "comic"
      )}`;
      // pushing to the search page
      ...
      if (
        value.comicInput.includes("https://www.marvel.com/comics/issue") ||
        value.comicInput.includes("http://www.marvel.com/comics/issue")
      ) {
        // getting the comic id
      ...
      const url = `/search?${getQuery(heroesString, "hero")}${getQuery(
        comicsString,
        "comic"
      )}`;
      // pushing to the search page
  ...
  return (
    <StyledHeader $isAdvancedHeader={isAdvancedSearch}>
      ...
      <span
        className="material-icons-outlined searchicon"
      ...
      <Tooltip tooltipContent={tooltipContent} position="bottom">
        <span
          tabIndex={0}
          className="material-icons-outlined advancedSearchIcon"
          data-testid=""
          onClick={flipIcon}
        >
          {size.width <= 640 ? "keyboard_arrow_down" : "keyboard_arrow_right"}
        </span>
      </Tooltip>
