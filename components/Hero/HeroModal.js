...
const HeroModal = ({ show, onClose, hero, explicitSearchParam }) => {
  ...
  const getComics = async (secSearchParams) => {
    ...
    if (secSearchParams) {
      ...
      comicsSearches = await axios.get(`${heroBaseURL}/${hero.id}/comics`, {
        params: {
  ...
  if (searchParams.comics?.length > 0 && comicList.length == 0) {
    ...
    modalBody = (
      <>
        ...
        <section>
          ...
          <ul>
            <li>
              Make sure that all words for hero and/or comics are spelled
              correctly.
            </li>
    ...
    modalBody = comicList.map((comic, index) => {
      const url = `${comic.thumbnail.path.replace("http:", "https:")}.${
    ...
    <StyledModal onClick={onClose}>
      <StyledOverlay onClick={(ev) => ev.stopPropagation()}>
        <ModalHeader>
          ...
          <h1>{hero.name}</h1>
          <p>
            {searchParams.comics?.length > 0
              ? `Showing results for "${searchParams.comics.join('" & "')}"`
              : "Latest comics for this hero"}
          </p>
        </ModalHeader>
