...
export const fetchData = async (heroParam, comicParam) => {
  ...
  let heroRes, comicRes, matchedHeroes;

  if (heroParam && comicParam) {
    return axios.get(`${heroBaseURL}/${heroParam}/comics`, {
      params: {
    ...
    heroRes = await axios.get(heroBaseURL, {
      params: {
        apikey: process.env.NEXT_PUBLIC_API_PUBLICKEY,
    ...
    matchedHeroes =
      heroRes.data.data.total > 0 ? heroRes.data.data.results : [];
    return matchedHeroes;
  } else if (comicParam) {
    comicRes = await axios.get(comicBaseURL, {
      params: {
        apikey: process.env.NEXT_PUBLIC_API_PUBLICKEY,
        titleStartsWith: comicParam,
        limit: 3,
